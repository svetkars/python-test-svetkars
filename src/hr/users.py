...
def update(user_info):
    print("Updating information about user '%s'" % user_info['name'])
    try:
        subprocess.call([
            'usermod',
            '-p',
        ...
        print("Failed to update information about '%s'" % user_info['name'])
        sys.exit(1)
...
def sync(users, existing_user_names=None):
    ...
    for user in users:
        if user['name'] not in existing_user_nmaes:
            add(user)
